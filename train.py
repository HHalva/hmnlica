...
def train(data_gen_dict, opt_dict, seed_dict,
          ...
          use_true_distrib=False, use_true_mix=False):
    ...
    # run with correct mix mlp (for testing)
    if use_true_mix:
        print("Forcing true mixing MLP")
    ...
    # use true source distribution parameters (for testing)
    if use_true_distrib:
        print("Forcing true source distribution parameters")
        mu_est = mu
        D_est = D
        A_est = A
    ...
    @jit
    def forward_backward_algo(logp_x, transition_matrix):
        # set T and K
        T, K = logp_x.shape
        ...
        # get initials state as left eig vec
        e_vals, e_vecs = np.linalg.eig(A_est_.T)
        one_eig_idx = np.argmin(np.abs(np.real(e_vals)-1.))
        #assert np.imag(e_vals[one_eig_idx]) == 0 !CANT JIT
        ev_one = np.real(e_vecs[:, one_eig_idx])
        pi_est_ = ev_one / ev_one.sum()
    ...
    @jit
    def viterbi_algo(logp_x, transition_matrix):
        # set up T and K
        T, K = logp_x.shape
        ...
        # initial state distrib as left eig vec
        e_vals, e_vecs = np.linalg.eig(A_est_.T)
        one_eig_idx = np.argmin(np.abs(np.real(e_vals)-1.))
        #assert np.imag(e_vals[one_eig_idx]) == 0 CANT JIT !!
        ev_one = np.real(e_vecs[:, one_eig_idx])
    ...
    def m_step(s_est, marg_posteriors, pw_posteriors):
        ...
        A_est = (hyprior-1+expected_counts) / (
            K*hyprior-K + expected_counts.sum(1).reshape(-1, 1))

        # update init state estimates (with prior)
        pi_est = marg_posteriors[0] + 1e-4
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 10 == 0 or epoch == num_epochs-1:
            ...
            logp_x_latest, _, _, s_est_latest = calc_likelihood(
                   params_latest, x_data, mu_est, D_est
               )

            # viterbi to estimate state prediction
            est_seq = viterbi_algo(logp_x_latest, A_est)
        ...
        s_corr_diag, s_est_sorted, sort_idx = matching_sources_corr(
            s_est_latest, s_data, method="pearson"
        )
        ...
        print("Epoch: [{0}/{1}]\t"
              ...
              "elapsed {time:.2f}".format(
                  ...
                  corr=mean_abs_corr, acc=clustering_acc, time=time()-tic))

        # visualize
        if viz_train and N == 2 and epoch % 100 == 0:
            visualize_train(s_data, s_est_latest, mu_est, D_est)
